...
# 3. Copy all compiled drivers and load order files into the root of the module
echo "  -> Injecting compiled drivers and order graph..."
cp "$KO_DIR"/*.ko "$MODULE_DIR/" 2>/dev/null || true
cp "$KO_DIR"/modules.order "$MODULE_DIR/" 2>/dev/null || true
cp "$KO_DIR"/modules.load "$MODULE_DIR/" 2>/dev/null || true
...
# 6. Generate action.sh (The elegant single-pass topological loader)
...
if ip link show 2>/dev/null | grep -qE "(wlan1|wlan2|wlan3).*UP"; then
    ...
    ORDER_FILE="$MODDIR/modules.order"
    [ -f "$MODDIR/modules.load" ] && ORDER_FILE="$MODDIR/modules.load"

    if [ -f "$ORDER_FILE" ]; then
        # Read the order file in REVERSE to cleanly unload dependencies
        tac "$ORDER_FILE" | while read -r mod_path; do
            ko_file=$(basename "$mod_path" .ko | tr '-' '_')
            rmmod "$ko_file" 2>/dev/null || true
        done
    else
        # Fallback hardcoded unload if order file is missing
    ...
    # Kill the stubborn stock modules just in case
    ...
    # Brutally tear down stock Android wireless modules first to clear the runway
    ...
    if [ -f "$ORDER_FILE" ]; then
        # ONE PASS. ZERO ERRORS.
        while read -r mod_path; do
            ko_file=$(basename "$mod_path")
            if [ -f "$MODDIR/$ko_file" ]; then
                insmod "$MODDIR/$ko_file" 2>/dev/null || true
            fi
        done < "$ORDER_FILE"
    else
        # Fallback brute-force loop if order file is somehow missing
